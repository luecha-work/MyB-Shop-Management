...
- `app/api/users/route.ts`: Admin/owner-only user list + form metadata + create/edit/delete/reset-password API. Passwords are generated by the UI for add-user or by the server for reset-password, then stored with `crypt($password, gen_salt('bf'))`.
