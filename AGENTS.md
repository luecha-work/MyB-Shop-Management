...
**Migration status (July 2026): UI migration is COMPLETE (100%).** All views from the original app have been ported 1:1 (layout shell + dashboard, POS, history, inventory, stockin) and subsequently converted to Ant Design components. Read-only PostgreSQL integration is now in place through Next.js API routes and is aligned with `db/new_schema.sql`; write/mutation workflows are still partially local state and remain the next backend milestone (see section 6).
...
- **Image Storage:** Product image uploads go to Supabase Storage public bucket `images`, folder `products/`, through the browser Supabase client. The current inventory product add/edit flow still updates local React state for product records until product mutation APIs are implemented.
...
- `app/api/stock-in/route.ts`: Stock-in history API backed by PostgreSQL `stock_in`; joins `products` through `stock_in.product_id` to display product names and applies STAFF branch scoping from the session.
...
- `lib/supabase/client.ts` / `lib/supabase/server.ts`: Supabase browser/server client helpers using `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`. Do not wire Supabase Auth middleware unless product requirements explicitly switch auth away from the current custom JWT/session flow. Browser uploads to Storage require Supabase bucket/policies to allow the intended authenticated/anon upload path.
