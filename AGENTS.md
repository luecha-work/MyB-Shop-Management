...
- `app/(dashboard)/users/page.tsx`: Manage-user page with desktop table plus mobile/iPad card list, add/edit user modal, reset-password confirm modal, copy temporary-password modal, and delete selected users. Users cannot delete themselves. Owner role forms hide branch selection and clear branch before save.
- `app/(dashboard)/branches/page.tsx`: Manage-branch page with desktop table plus mobile/iPad card list, view/edit actions, add/edit branch modal, detail modal showing assigned users/roles, and delete selected branches. Success/error operation feedback uses floating antd `Alert` components at the top-right under the app header and auto-dismisses.
