...
- `app/api/products/route.ts`: Product list/add/edit API backed by PostgreSQL `products`. `POST` creates products with an auto-generated numeric `product_code` stored as text (`1`, `2`, `3`, ...), and admin/owner-only `PATCH { action: 'update-product' }` updates product fields plus `products.image_url`; `PATCH { action: 'update-image' }` remains available for image-only updates.
